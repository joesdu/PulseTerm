...
**上下文按窗口自动裁剪**(`Chat/RequestContext.cs`,纯函数、可单测):每轮把
`系统提示词 + 历史` 装配成真正要发的消息序列。装不下就从最早的往后丢,**只在用户消息处下刀**
...
* **决定"这条消息怎么发"的东西挨着输入框** —— 模型选择、Agent 开关与发送按钮同处输入框那个
  描边容器内(`InputToolbar`);容器正下方一条细行(`InputStatusBar`)左边是 Agent 的审批模式、
  右边是 token 用量。顶栏只留 SSH 会话选择与历史/新会话/设置。
...
- 每台服务器可填一份**屏蔽清单**(每行一个服务器上的原始工具名):有些 MCP 一口气给几十个工具,
  全塞给模型既占上下文又容易被误调。"测试"按钮列出来的就是可填的名字。
