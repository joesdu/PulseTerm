...
public sealed class Loc(string locale)
{
    ...
    private static readonly Dictionary<string, string[]> Table = new()
    {
        ...
        ["McpDisabledTools"] = ["Hidden tools (one per line, server-side names)", "屏蔽的工具(每行一条,填服务器上的原始名)", "遮蔽的工具(每行一條,填伺服器上的原始名)", "非表示にするツール(1 行に 1 つ、サーバー側の名前)", "숨길 도구(줄마다 하나, 서버 쪽 원래 이름)"],
        ...
        ["PanelWidth"] = ["Side pane width (%)", "侧栏宽度(%)", "側欄寬度(%)", "サイドペインの幅(%)", "사이드 패널 너비(%)"],
        ["PanelWidthHint"] = ["Width of the right-hand column when the chat opens as a tab (15–85). Drag the splitter any time; the dragged width is not saved here.", "以标签页打开时右侧那一栏的初始宽度(15–85)。之后拖分割条随时可改,拖出来的宽度不会写回这里。", "以標籤頁開啟時右側那一欄的初始寬度(15–85)。之後拖分割條隨時可改,拖出來的寬度不會寫回這裡。", "タブとして開いたときの右カラムの初期幅(15–85)。分割バーでいつでも変更できますが、その幅はここに保存されません。", "탭으로 열 때 오른쪽 칸의 초기 너비(15–85). 분할선은 언제든 드래그할 수 있으며, 그 너비는 여기에 저장되지 않습니다."],
