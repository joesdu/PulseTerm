...
public sealed class Loc(string locale)
{
    ...
    private static readonly Dictionary<string, string[]> Table = new()
    {
        ...
        ["WebSearxHint"] = ["SearXNG is a metasearch engine: it queries Google/Bing/DDG for you and merges the results, so no API key is involved. Run one with \"docker run -d -p 8080:8080 searxng/searxng\" and point this at it, e.g. http://127.0.0.1:8080 . Its settings.yml must list \"json\" under search.formats — the default is html only, and the instance answers 403 without it.", "SearXNG 是元搜索引擎:它替你去问 Google/Bing/DDG 再合并结果,全程不涉及任何 API Key。一句 docker run -d -p 8080:8080 searxng/searxng 就能起一台,地址填成形如 http://127.0.0.1:8080。它的 settings.yml 必须把 json 列进 search.formats —— 默认只有 html,不加这条实例会直接回 403。", "SearXNG 是元搜尋引擎:它替你去問 Google/Bing/DDG 再合併結果,全程不涉及任何 API Key。一句 docker run -d -p 8080:8080 searxng/searxng 就能起一台,位址填成形如 http://127.0.0.1:8080。它的 settings.yml 必須把 json 列進 search.formats —— 預設只有 html,不加這條實例會直接回 403。", "SearXNG はメタ検索エンジンです(Google/Bing/DDG に代理で問い合わせて結果をまとめるので、API キーは不要)。docker run -d -p 8080:8080 searxng/searxng の 1 行で用意でき、http://127.0.0.1:8080 のように指定します。settings.yml の search.formats に json を追加してください — 既定は html のみで、無いとインスタンスは 403 を返します。", "SearXNG는 메타 검색 엔진입니다(Google/Bing/DDG에 대신 질의해 결과를 합치므로 API 키가 필요 없습니다). docker run -d -p 8080:8080 searxng/searxng 한 줄로 띄우고 http://127.0.0.1:8080 처럼 지정하세요. settings.yml의 search.formats에 json을 추가해야 합니다 — 기본값은 html뿐이라 없으면 인스턴스가 403을 반환합니다."],
