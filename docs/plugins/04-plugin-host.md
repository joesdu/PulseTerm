...
> **实现注记(2026-08)**:`src/VelaShell.PluginHost` 已落地 —— 每插件一进程、
> 环境变量携带管道名/令牌(不进命令行)、可收集 ALC 装载、限时激活/停用、
> 父进程守望防孤儿、stdout/stderr 重定向宿主 Trace、内建 Avalonia(插件完整 UI)。
> 崩溃处置亦已落地:心跳(默认 30s,连续两次无应答判挂死强杀)+ 意外退出按
> 退避序列自动重启(默认 1s→5s→30s,5 分钟窗口内超限判 Failed 等待用户处置)。
> 空闲回收亦已落地:`idlePolicy: "recyclable"` + 连续空闲(无 RPC 往来且无打开面板,
> 默认 15 分钟)即停用回收进程,清单占位命令留守、再触发即重新拉起。
