...
> 目前重心在 **Token 与图标基建**：控件本体只有图标与资源监视用的几个绘制控件，业务性视图（状态栏、标签条、面板等）仍在应用入口项目 `VelaShell` 内以 XAML 组合实现，引用本项目的 Token 与图标。设计稿要求的逐帧自定义控件库仍是持续演进方向（见 `velashell-docs zh/host/架构设计.md` §6）。
