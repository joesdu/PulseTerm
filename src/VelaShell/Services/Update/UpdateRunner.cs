...
/// <summary>
/// 外置换版进程的入口。跑这段代码的不是安装好的应用,而是暂存目录里那份解包完毕的新版应用
/// (见 <see cref="UpdateApplier.TryHandOffToExternalUpdater" />)—— 更新包本身就是一套
/// 自包含的完整产物,原地跑起来即可,因此不必随包分发额外的更新器,也就没有"更新器自己
/// 怎么更新""小体积未签名 exe 被杀软当木马"这些麻烦。
/// <para>
/// 它做的事很少:等主进程退出 → 换版 → 拉起应用目录里的主程序 → 自己退出。
/// 换版发生在应用完全退出之后,应用目录里没有任何文件被占用,覆盖必然成功,
/// 从根上消除了"旧文件删不掉"这一类残留。
/// </para>
/// <para>
/// <b>约束:这条路径只用 BCL,不碰 Avalonia / Skia / SQLite,且必须在 <c>Program.Main</c>
/// 最顶端、任何 Avalonia 初始化之前被调用。</b>它是个无界面的短命进程:起 UI 既没意义,
/// 又会把 GPU/字体/数据库这些重家伙拖进来,平白拉长用户盯着"正在更新"的那几秒;
/// 更要紧的是它绝不能对应用目录里的任何文件持有句柄 —— 那正是马上要被覆盖的东西。
/// 它自己的程序集全部解析自解包目录,换版只写应用目录,两边不相干。
/// </para>
/// </summary>
