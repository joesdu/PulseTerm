...
/// <summary>
/// 把终端内文件传输(ZMODEM / XMODEM / YMODEM)的进度桥接到既有的文件传输面板(toast):
/// 每个 <see cref="FileTransferItem" /> 映射为一个 <see cref="TransferTask" /> 并驱动
/// <see cref="FileTransferViewModel" />。所有回调都从引擎的后台线程经
/// <see cref="Dispatcher.UIThread" /> 编组到 UI 线程后再改动视图模型。
/// </summary>
internal sealed class TerminalTransferObserver(FileTransferViewModel fileTransfer) : IFileTransferObserver
{
    ...
    // 本次会话的方向。面板据此区分上传 / 下载 —— 之前一律写死 Download,
    // 上传(远端 rz / rb)在面板里被显示成下载,图标与文案都是反的。
    private volatile bool _isUpload;

    /// <inheritdoc />
    public void OnSessionStarted(FileTransferSession session) =>
        _isUpload = session.Direction == FileTransferDirection.Send;

    /// <inheritdoc />
    public void OnFileStarted(FileTransferItem item)
    {
        ...
        Dispatcher.UIThread.Post(() =>
        {
            ...
            {
                Id = id,
                Type = isUpload ? TransferType.Upload : TransferType.Download,
    ...
    /// <inheritdoc />
    public void OnFileProgress(FileTransferItem item)
    {
    ...
    /// <inheritdoc />
    public void OnFileCompleted(FileTransferItem item)
    {
    ...
    /// <inheritdoc />
    public void OnFileSkipped(FileTransferItem item)
    {
        _startedAt.TryRemove(item.Id, out _);
        _lastProgressTick.TryRemove(item.Id, out _);
        Guid id = item.Id;
    ...
    /// <inheritdoc />
    public void OnSessionFailed(FileTransferSession session, Exception? error)
    {
        _ = error;
    ...
    private double ComputeSpeed(FileTransferItem item)
    {
