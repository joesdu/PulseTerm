...
internal sealed class FolderTransferFileSink(
    Func<TransferFolderPromptRequest, CancellationToken, Task<string?>> pickFolderAsync,
    Func<Task<AppSettings>> getSettingsAsync) : IFileTransferSink, IAsyncDisposable
{
    private readonly Func<TransferFolderPromptRequest, CancellationToken, Task<string?>> _pickFolderAsync =
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return (TransferFileDisposition.Skip, 0);
        }
        ...
        return (TransferFileDisposition.Accept, 0);
    }

    /// <inheritdoc />
    public async ValueTask WriteAsync(FileTransferItem item, ReadOnlyMemory<byte> data, CancellationToken cancellationToken)
    {
    ...
    /// <inheritdoc />
    public async ValueTask CompleteAsync(FileTransferItem item, CancellationToken cancellationToken)
    {
    ...
    /// <inheritdoc />
    public async ValueTask FailAsync(FileTransferItem item, Exception? error, CancellationToken cancellationToken)
    {
        _ = error;
        _ = cancellationToken;
        if (_streams.Remove(item.Id, out FileStream? stream))
